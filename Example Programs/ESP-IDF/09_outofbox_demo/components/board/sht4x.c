...
esp_err_t sht4xInit(void)
{
    ESP_ERROR_CHECK(i2c_master_get_bus_handle(0, &master_handle));
    if(master_handle != NULL)
    {
        sht4x_handle = sht4x_device_create(master_handle, SHT4X_I2C_ADDR_0, 400000);
    }
    else
    {
        return ESP_FAIL;
...
esp_err_t sht4xTest(char *output_buf)
{
    esp_err_t err;
    float temperature, humidity;
    err = i2c_master_probe(master_handle, SHT4X_I2C_ADDR_0, 200);
    if(err == ESP_OK)
    {
        err = sht4x_start_measurement(sht4x_handle, SHT4X_CMD_READ_MEASUREMENT_HIGH);
        vTaskDelay(pdMS_TO_TICKS(50));  
        err = sht4x_read_measurement(sht4x_handle, &temperature, &humidity);
        if (err == ESP_OK) 
        {
            sprintf(output_buf,"test_success Tem: %.2f C,Hum: %.2f %%", temperature,humidity);
            return  ESP_OK;
        } 
        else 
        {
