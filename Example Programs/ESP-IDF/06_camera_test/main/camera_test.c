...
void esp_camera_port_init(i2c_port_num_t i2c_port) 
{
    ...
    config.frame_size = FRAMESIZE_VGA; // Capture frame size (VGA = 640x480 pixels)
...
void camera_task(void *arg)
{
    ...
    img_dsc.header.w = 640;            // Image width in pixels (matches camera output width)
    img_dsc.header.h = 480;            // Image height in pixels (matches camera output height)
    img_dsc.data_size = 640 * 480 * 2; // Total size of image data in bytes (640x480 resolution, 2 bytes per pixel e.g., RGB565)
...
void app_main(void)
{
    ...
    if (lvgl_port_lock(0))
    {
        ...
        {     
            ...
            //Get the handle of the default LVGL display device (for subsequent rotation configuration)
            lv_disp_t * cfg_disp = lv_disp_get_default();
            if(cfg_disp != NULL)
            {
                lv_disp_set_rotation(cfg_disp, LV_DISPLAY_ROTATION_180);
            } 
