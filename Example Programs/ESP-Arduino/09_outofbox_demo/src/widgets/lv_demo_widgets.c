 ...
 *
 * Arduino port of the customized LVGL widgets demo used by the ESP-IDF
 * 09_outofbox_demo example. The UI content and image assets are kept aligned
 * with the ESP-IDF version; only platform-specific board includes were removed.
 *
 */
...
{
    /* Select the same responsive layout branch as the ESP-IDF demo. */
...
static void profile_create(lv_obj_t * parent)
{
    ...
    /* Custom Nextion avatar copied from the ESP-IDF widgets assets. */
    ...
    if(code == LV_EVENT_FOCUSED) {
        if(lv_indev_get_type(lv_indev_get_act()) == LV_INDEV_TYPE_POINTER) {
            if(calendar == NULL) {
                ...
                #if NEXTION_3_5
                if(disp_size == DISP_SMALL) lv_obj_set_size(calendar, 280, 320);
                #endif
                #if NEXTION_2_8
